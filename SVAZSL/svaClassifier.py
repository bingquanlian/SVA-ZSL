...
class CLASSIFIER:
    ...
    def __init__(self, opt, _train_X, _train_Y, data_loader, test_seen_feature,test_unseen_feature, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20,
        ...
        if self.cuda:
            self.model.to(opt.ngpu)
            self.criterion.to(opt.ngpu)
            self.input = self.input.to(opt.ngpu)
            self.label = self.label.to(opt.ngpu)
    ...
    def val_gzsl(self, test_X, test_label):
        ...
        with torch.no_grad():
            for i in range(0, ntest, self.batch_size):
                ...
                if self.cuda:
                    output = self.model(test_X[start:end].to(self.opt.ngpu))
                else:
